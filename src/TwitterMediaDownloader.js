...
// @downloadURL https://update.greasyfork.org/scripts/528890/XTwitter%20%E3%83%A1%E3%83%87%E3%82%A3%E3%82%A2%E4%B8%80%E6%8B%AC%E3%83%80%E3%82%A6%E3%83%B3%E3%83%AD%E3%83%BC%E3%83%80%E3%83%BC%EF%BC%88iPhoneAndroid%20%E5%AF%BE%E5%BF%9C%29.user.js
// @updateURL https://update.greasyfork.org/scripts/528890/XTwitter%20%E3%83%A1%E3%83%87%E3%82%A3%E3%82%A2%E4%B8%80%E6%8B%AC%E3%83%80%E3%82%A6%E3%83%B3%E3%83%AD%E3%83%BC%E3%83%80%E3%83%BC%EF%BC%88iPhoneAndroid%20%E5%AF%BE%E5%BF%9C%29.meta.js
// ==/UserScript==
/*jshint esversion: 11 */
...
(function () {
    ...
    // === Aria2 RPC Settings ===
    /**
     * Aria2 RPC configuration for automatic download task creation
     */
    const enableAria2Download = true; // Set to true to enable Aria2 RPC download
    const aria2RpcUrl = 'http://10.0.0.1:6800/jsonrpc'; // Aria2 RPC address
    const aria2RpcSecret = '88888888'; // Aria2 RPC secret token
    const aria2DownloadPath = '/mnt/data_sda1/download/X'; // Default download directory
     ...
     */
    const callAria2Rpc = async (method, params = []) => {
        const rpcPayload = {
            jsonrpc: '2.0',
            method: method,
            id: Date.now().toString(),
            params: aria2RpcSecret ? [`token:${aria2RpcSecret}`, ...params] : params
        };

        console.log(`🔧 [Aria2 RPC] Calling ${method}:`, params);

        try {
            const response = await gmFetch(aria2RpcUrl, {
                method: 'POST',
                headers: {
            ...
            if (result.error) {
                console.error(`❌ [Aria2 RPC] Error in ${method}:`, result.error);
                throw new Error(`Aria2 RPC Error: ${result.error.message}`);
            }

            console.log(`✅ [Aria2 RPC] ${method} success:`, result.result);
            return result.result;
        } catch (error) {
            console.error(`❌ [Aria2 RPC] Failed to call ${method}:`, error);
            throw error;
        }
    };
    ...
    const checkUrlAccessibility = async (url) => {
        console.log(`🔍 [URL Check] Testing: ${url.substring(0, 80)}...`);
        try {
            ...
            if (response.ok) {
                console.log(`✅ [URL Check] Accessible (Status: ${response.status})`);
                return true;
            } else {
                console.warn(`⚠️ [URL Check] Not accessible (Status: ${response.status})`);
                return false;
            }
        } catch (error) {
    ...
    /**
     * Add download task to Aria2
     */
    const addToAria2 = async (url, filename, options = {}) => {
        // Check URL accessibility first
        const isAccessible = await checkUrlAccessibility(url);
        if (!isAccessible) {
            console.warn(`⚠️ [Aria2] Skipping inaccessible URL: ${filename}`);
            return null;
        }
        ...
        if (aria2DownloadPath) {
            aria2Options.dir = aria2DownloadPath;
        }

        try {
            const gid = await callAria2Rpc('aria2.addUri', [[url], aria2Options]);
            console.log(`📥 [Aria2] Task added successfully!`);
            console.log(`   GID: ${gid}`);
            console.log(`   File: ${filename}`);
            console.log(`   Path: ${aria2DownloadPath || 'default'}`);
            return gid;
        } catch (error) {
            console.error(`❌ [Aria2] Failed to add task for ${filename}:`, error.message);
            return null;
        }
    };
    ...
    const testAria2Connection = async () => {
        console.log(`🔌 [Aria2] Testing connection to ${aria2RpcUrl}...`);
        try {
            const version = await callAria2Rpc('aria2.getVersion');
    ...
    // Test Aria2 connection on script startup
    if (enableAria2Download) {
        testAria2Connection().then(connected => {
            if (connected) {
                console.log('\n✨ [Twitter Media Downloader] Aria2 integration enabled and ready!');
            } else {
                console.warn('\n⚠️  [Twitter Media Downloader] Aria2 connection failed. Will fall back to browser download.');
            }
        });
    }
    ...
    const fetchBookmarkSearchTimeline = async (userId, postTime) => {
        ...
        const res = await gmFetch(url, { headers, responseType: 'json' });
        if (!res.ok) throw new Error(`BookmarkSearchTimeline failed: ${res.status}`);
    ...
    const getAlreadyBookmarkedMessage = () => {
        const lang = getCurrentLanguage();
        if (lang === 'ja') {
            return window.confirm('この投稿は既にダウンロードされています。\nダウンロードを続行しますか？');
        } else {
            return window.confirm('This post is already downloaded.\nDo you want to continue downloading?');
        }
    };
    ...
    const downloadMedia = async (imageURLs, gifURLs, videoURLs, filenameElements, btn_down, allMediaURLs, cell, bookmarkPromise) => {
        ...
        console.log('📥 Twitter Media Download URLs');
        console.log('='.repeat(80));
        console.log('🆔 Post ID:', filenameElements.postId);
        console.log('👤 User ID:', filenameElements.userId);
        console.log('📅 Post Time:', filenameElements.postTime);
        console.log('-'.repeat(80));

        // 输出图片URL
        if (imageURLs.length > 0) {
            console.log('🖼️  Images (' + imageURLs.length + '):', imageURLs);
        ...
        // 输出GIF URL
        if (gifURLs.length > 0) {
            console.log('🎞️  GIFs (' + gifURLs.length + '):', gifURLs);
            gifURLs.forEach((url, idx) => {
                console.log(`  [${idx + 1}] ${url}`);
            });
        }
        ...
        if (videoURLs.length > 0) {
            console.log('🎬 Videos (' + videoURLs.length + '):', videoURLs);
        ...
        // 输出所有URL（用于Aria2）
        console.log('-'.repeat(80));
        console.log('📋 All URLs (for Aria2):');
        allMediaURLs.forEach((url, idx) => {
            const mediaInfo = getMediaInfoFromUrl(url);
            const filename = generateFilename(filenameElements, mediaInfo.typeLabel, idx + 1, mediaInfo.ext);
            console.log(`  ${url}`);
        ...
        // 生成Aria2命令
        console.log('-'.repeat(80));
        console.log('⚡ Aria2 Download Commands:');
        allMediaURLs.forEach((url, idx) => {
            const mediaInfo = getMediaInfoFromUrl(url);
            const filename = generateFilename(filenameElements, mediaInfo.typeLabel, idx + 1, mediaInfo.ext);
            console.log(`aria2c "${url}" -o "${filename}"`);
        });
        console.log('='.repeat(80));
        // === 控制台输出结束 ===
        ...
        // === Aria2 自动下载 ===
        if (enableAria2Download) {
            console.log('\n🚀 [Aria2 Auto Download] Starting...');
            console.log('-'.repeat(80));

            const downloadResults = [];

            for (let idx = 0; idx < allMediaURLs.length; idx++) {
                const url = allMediaURLs[idx];
                const mediaInfo = getMediaInfoFromUrl(url);
                const filename = generateFilename(filenameElements, mediaInfo.typeLabel, idx + 1, mediaInfo.ext);

                console.log(`\n[${idx + 1}/${allMediaURLs.length}] Processing: ${filename}`);

                try {
                    const gid = await addToAria2(url, filename);
                    if (gid) {
                        downloadResults.push({ success: true, filename, gid });
                    } else {
                        downloadResults.push({ success: false, filename, error: 'URL not accessible' });
                    }
                } catch (error) {
                    downloadResults.push({ success: false, filename, error: error.message });
                }
            }

            // 输出下载结果统计
            console.log('\n' + '='.repeat(80));
            console.log('📊 [Aria2 Download Summary]');
            console.log('='.repeat(80));
            const successCount = downloadResults.filter(r => r.success).length;
            const failCount = downloadResults.length - successCount;
            console.log(`✅ Success: ${successCount}/${downloadResults.length}`);
            console.log(`❌ Failed: ${failCount}/${downloadResults.length}`);

            if (successCount > 0) {
                console.log('\n📥 Added to Aria2:');
                downloadResults.filter(r => r.success).forEach(r => {
                    console.log(`  ✓ ${r.filename} (GID: ${r.gid})`);
                });
            }
    ...
    const createDownloadButton = async (cell) => {
        ...
        btn_down.onclick = async () => {
            ...
            let buttonStateBeforeClick = btn_down.classList.contains('completed') ? 'completed' : 'download';
            ...
            downloadMedia(mediaData.imageURLs, mediaData.gifURLs, mediaData.videoURLs, filenameElements, btn_down, mediaUrls, cell, bookmarkPromise);
        };
